...
On **Vercel**, NationForge **must** have `DATABASE_URL` set for every environment where you create or join games: serverless instances cannot write the local `.data/` JSON store. Missing DB env produces a clear JSON error from `POST /api/nationforge/sessions` (and the new-session page surfaces `error` / `message`). Preview deployments use the same env as configured in the Vercel project (add `DATABASE_URL` for Preview if needed).
